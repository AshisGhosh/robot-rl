...
class Args:
    ...
    sim_backend: str = "gpu"
    """the simulation backend of the environment"""
    shader: str = "default"
    """the shader of the environment"""
...
if __name__ == "__main__":
    ...
    if args.exp_name is None:
        ...
        run_name = (
            f"{args.output_dir.split('/')[-2]}_{os.path.basename(args.output_dir)}"
        )
    else:
    ...
    if args.capture_video:
        ...
        if args.save_train_video_freq is not None:
            save_video_trigger = (  # noqa: E731
                lambda x: (x // args.num_steps) % args.save_train_video_freq == 0
            )
            envs = RecordEpisode(
                envs,
                output_dir=f"{args.output_dir}/train_videos",
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        if args.save_model and iteration % args.eval_freq == 1:
            ...
            model_path = f"{args.output_dir}/checkpoints/ckpt_{iteration}.pt"
    ...
    if not args.evaluate:
        if args.save_model:
            model_path = f"{args.output_dir}/checkpoints/final_ckpt.pt"
            torch.save(agent.state_dict(), model_path)
            print(f"model saved to {model_path}")
        writer.close()
    envs.close()
